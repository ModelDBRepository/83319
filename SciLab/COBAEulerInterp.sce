...
// with linear spike time interpolation
//
...
while t<duration
  ...
  // Linear spike time interpolation
  h=(Vt-oldV(spikes))./(S(spikes,1)-oldV(spikes));
  S(spikes,1)=Vr+((1-h)*dt/taum).*(-Vr-S(spikes,2).*(Vr-Ee)-S(spikes,3).*(Vr-Ei));
